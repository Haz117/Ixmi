...
export const OfflineProvider = ({ children }) => {
  ...
  const loadDataFromFirebase = useCallback((callback, options = {}) => {
    ...
    console.log('🔄 Intentando conectar a colección "seccionales"...');

    // Configurar opciones del listener para mejorar la conectividad
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (isUnsubscribed) return; // Evitar procesar si ya se desconectó
        ...
        console.log('🔍 DEBUG: From cache:', snapshot.metadata.fromCache);
        console.log('🔍 DEBUG: Has pending writes:', snapshot.metadata.hasPendingWrites);
        
        // Solo procesar si no son solo cambios de metadata
        if (snapshot.metadata.hasPendingWrites) {
          console.log('🔍 DEBUG: Ignorando cambios con writes pendientes');
          return;
        }
